...
const STATUSES = ['received', 'printing', 'ready', 'delivered', 'cancelled'];

export default function PrintCoversAdmin() {
  ...
  function load() {
    setLoading(true);
    api.get('/print-covers', { params: filter ? { status: filter } : {} }).then((res) => {
      setRequests(res.data);
      const d = {};
      res.data.forEach((r) => {
        d[r._id] = { status: r.status, price: r.price || 0, adminNotes: r.adminNotes || '' };
      });
      setDrafts(d);
  ...
  async function save(id) {
    try {
      await api.put(`/print-covers/${id}/status`, drafts[id]);
      toast.success('Print cover request updated');
      load();
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-navy-900">Photo Print Cover Requests</h1>
      ...
      {loading ? (
        ...
        <p className="text-navy-400 text-center py-16">No print cover requests found.</p>
      ) : (
            ...
            <div key={r._id} className="bg-white border border-navy-100 rounded-xl p-5 shadow-sm">
              <div className="flex flex-wrap justify-between items-start gap-3 mb-3">
                <div className="flex gap-4">
                  <a href={resolveImage(r.photo)} target="_blank" rel="noreferrer">
                    <img src={resolveImage(r.photo)} alt="Print request" className="h-20 w-20 object-cover rounded-lg border border-navy-100 shrink-0" />
                  </a>
                  <div>
                    ...
                    {r.notes && <p className="text-sm text-navy-600 mt-1">Notes: {r.notes}</p>}
              ...
              <div className="grid gap-3 sm:grid-cols-3 items-end">
                ...
                <div>
                  ...
                  <input type="number" min="0" value={drafts[r._id]?.price}
                    onChange={(e) => updateDraft(r._id, 'price', e.target.value)}
                    className="w-full border border-navy-200 rounded-md px-3 py-2 text-sm" />
                </div>
