...
const STATUSES = ['pending', 'contacted', 'offer-made', 'purchased', 'rejected'];
...
export default function SellRequestsAdmin() {
  ...
  function load() {
    setLoading(true);
    api.get('/sell-requests', { params: filter ? { status: filter } : {} }).then((res) => {
      setRequests(res.data);
      const d = {};
      res.data.forEach((r) => {
        d[r._id] = { status: r.status, offeredPrice: r.offeredPrice || 0, adminNotes: r.adminNotes || '' };
      });
      setDrafts(d);
  ...
  async function save(id) {
    try {
      await api.put(`/sell-requests/${id}/status`, drafts[id]);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-navy-900">Sell Requests</h1>
      ...
      {loading ? (
        ...
        <p className="text-navy-400 text-center py-16">No sell requests found.</p>
      ) : (
            ...
            <div key={r._id} className="bg-white border border-navy-100 rounded-xl p-5 shadow-sm">
              <div className="flex flex-wrap justify-between items-start gap-3 mb-3">
                <div>
                  ...
                  <p className="text-sm text-navy-600 mt-1">Condition: {r.condition} · Expected: ₹{r.expectedPrice}</p>
                  {r.description && <p className="text-sm text-navy-500 mt-1">{r.description}</p>}
              ...
              <div className="grid gap-3 sm:grid-cols-3 items-end">
                ...
                <div>
                  <label className="block text-xs font-medium text-navy-500 mb-1">Offered Price (₹)</label>
                  <input type="number" min="0" value={drafts[r._id]?.offeredPrice}
                    onChange={(e) => updateDraft(r._id, 'offeredPrice', e.target.value)}
                    className="w-full border border-navy-200 rounded-md px-3 py-2 text-sm" />
                </div>
